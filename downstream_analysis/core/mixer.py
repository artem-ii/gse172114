...
class Mixer:
        ...
        for cell in cells_to_mix:
            ...
            for i in range(num_av):
                ...
                if self.proportions is not None:
                    ...
                    if len(list(specified_subtypes)) > 1:
    ...
    def change_subtype_proportions(self, cell: str, cells_index: pd.Index) -> pd.Index:
        ...
        min_num = min(self.proportions.loc[list(specified_subtypes)])

        subtype_proportions = {cell: dict(self.proportions.loc[list(specified_subtypes)])}
